...
const CodeView = ({
  ...
  const copyToClipboard = (component, name, copyClassType, className) => {
    const eventLabelClass =
      copyClassType && className
        ? [copyClassType, className.toLowerCase()].join("=")
        : null;
    const eventLabelComponent = [
      component,
      name.split(" ").join("-").toLowerCase(),
    ].join("-");
    const eventLabel = eventLabelClass
      ? [eventLabelComponent, eventLabelClass].join("-")
      : eventLabelComponent;
  ...
  return (
    <>
      ...
      >
              ...
              <div className="w-code-actions v-flex-wrap">
                ...
                {exampleLink ? (
                  <Link
                    ...
                    newWindow
                    className="v-link"
                    aria-label={`Example of ${name}; Opens in a new tab`}
                  >
                    Example
        ...
        {showDescription ? (
          <div
            className="w-code-description v-surface"
            aria-hidden="false"
            id={`show-description-${paramCase(name)}`}
            ...
            <>
              {showProgress ? (
                <div className=" w-code-loading">
                  <progress
                    className="v-progress v-progress-bar"
                    id="progress-bar-indeterminate"
                  ></progress>
                </div>
              ) : null}
              <iframe
                src={`${PUBLIC_URL}/${exampleLink}`}
                style={{
                  minBlockSize: "200px",
                  inlineSize: "100%",
                  padding: "0px",
                }}
                className="w-code-demo"
                title={`Interaction example for ${component} example "${name}"`}
                ref={iframeRef}
              ></iframe>
            </>
          ) : (
            <div
              className={["w-code-demo"].join(" ").trim()}
        ...
        <div className="w-code-footer">
          <Accordion
            ...
            aria-label={`${
              showCode ? "Hide" : "Show"
            } code for ${component} ${name} example`}
            open={showCode}
          ...
          >
            {toRender ? (
              <div className="w-code">
                <Highlight
                  ...
                  language="html"
                  theme={
                    globalState.theme.indexOf("dark") >= 0 ||
                    globalState.mode === "dark"
                      ? themes.vsDark
                      : themes.github
                  }
                >
                      ...
                      <div className="w-code-copy-container">
                        <CopyToClipboard text={toRender}>
                          ...
                          >
                            ...
                            <span
                              className="v-typography-label-active"
            ...
            {classes &&
              ...
              classes.map((x, i) => {
                ...
                return (
                  <div className="w-code-context" key={i}>
