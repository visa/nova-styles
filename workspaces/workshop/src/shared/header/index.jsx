...
const Header = React.forwardRef((props, ref) => {
  ...
  useEffect(() => {
    if (isHybrid && !localStorage.getItem("theme-preference")) {
  ...
  // Initialize theme on load, use system preference if no user preference is set
  useEffect(() => {
    if (isHybrid) {
  ...
  const handleThemeChange = (e) => {
    ...
    // reset mode if the new theme does not support light/dark toggling
    if (
      _theme.indexOf("hybrid") < 0 &&
      _theme !== "visa" &&
      _theme !== "default" &&
      _theme !== "visa-alt"
    ) {
      ...
      // if the theme supports toggling, use the saved preference
      const savedPreference = localStorage.getItem("w-mode");
      dispatch({
        type: actionTypes.setMode,
        payload: savedPreference,
      });
  ...
  useEffect(() => {
    ...
    if (isHybrid) {
      // Only set data-theme if user has made an explicit choice
  ...
  useEffect(() => {
    setIsHybrid(
      globalState.theme.indexOf("hybrid") >= 0 ||
        globalState.theme === "visa" ||
        globalState.theme === "default" ||
        globalState.theme === "visa-alt"
    );
  }, [globalState.theme]);

  const toggleSideNav = () => {
    dispatch({
      type: actionTypes.setSideNav,
      payload: !globalState.sideNavOpen,
    });
  };
    ...
    >
      <div className="v-flex v-gap-10">
        <button
          ...
          className="v-button v-button-icon v-button-tertiary v-desktop-container-hide"
        ...
        >
          ...
          >
            <use
              href={
                globalState.sideNavOpen ? "#visa-close-tiny" : "#visa-menu-tiny"
              }
            ></use>
          </svg>
        </button>
        <a
          href={`${PUBLIC_URL || "/"}`}
          className="v-flex v-mx-auto v-flex-shrink v-link-unstyled v-flex-grow"
          aria-label="Visa Workshop/HTML and CSS Styles Home"
          id="get-started"
        >
          <svg
            width="71"
          ...
          <span className="v-mobile-container-hide v-ml-16 v-flex-grow v-mr-auto v-typography-headline-3 v-nav-app-name">
            VPDS Nova <span className="v-mx-8">|</span> Styles
          </span>
        </a>
      </div>
      <div className="v-flex v-gap-10 v-align-items-center v-justify-content-end">
        ...
        */}
        ...
        <fieldset
          className="v-flex v-flex-row v-align-items-center v-mobile-container-hide"
          aria-labelledby="select-theme-label"
        >
          <label
            className="v-label"
            htmlFor="select-theme-field"
        ...
        {isHybrid ? (
          <fieldset className="v-flex v-flex-row">
            <label
              className="v-switch-label v-label v-mr-8"
              htmlFor="select-mode-field"
            >
              Dark mode
            </label>
            <input
              className="v-switch"
              id="select-mode-field"
              name="default-switch"
              role="switch"
              type="checkbox"
              onChange={handleModeChange}
              checked={globalState.mode === "dark"}
            />
          </fieldset>
        ) : null}
