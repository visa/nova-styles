...
const WorkshopTheme = ({ type, item, ...props }) => {
  ...
  const [relationalGraphSegment, setRelationalGraphSegment] = useState('palette'); 
  const [hoverLink, setHoverLink] = useState({}); 
  const [selectedTab, setSelectedTab] = useState('config'); 

  const cleanVar = (x) => (x.indexOf('var') === 0 
  ...
  const buildRelationalGraph = useCallback((data, segment) => (
    data 
      ? Object.keys(data)
        .filter(x => 
    ...
    <main className="w-content" id="content">
      <div className="w-area v-p-30">
        <div className="v-flex v-flex-row v-justify-content-between v-align-items-center v-gap-12">
          ...
          <button className="v-button" disabled={paramCase(globalState.theme) === paramCase(item)} onClick={handleApplyTheme}>
            Apply this theme
          </button>
          <Link className="v-button v-button-secondary" route={'https://bookmarks.visa.com/vpds-theme-app'} newWindow>
        ...
        <ul role="tablist" className="v-tabs v-tabs-horizontal">
          ...
          {Object.keys(docs.entries.themes[item].variables).filter((x) => x.indexOf('--elevation') >= 0).length > 0
            ? <li className="v-tab" role="none">
              <button 
                className="v-button v-button-tertiary" 
                role="tab" 
...

        {
          ...
          ? <div className="v-card v-card-body">
            <div className="v-message v-section">
              <svg focusable="false" aria-hidden="true" className="v-icon v-icon-visa v-icon-low v-message-icon"
              viewBox="0 0 24 24">
                  <use href="#visa-information-low"></use>
              </svg>
              <div className="v-message-content">
                For more information, see our <Link route="#configuration">Theming Configuration guidelines</Link>.
              </div>
            </div>
            <br />
            <table className="v-table v-table-border-block" border="1" width="100%">
              <thead align="left">
                <tr className="v-tr">
                    ...
                    ? <tr className="v-tr" key={i}>
                      ...
                      <td className="v-td">
                        <code 
                          className={[
                            "w-code v-badge", 
                            docs.entries.themes[item].variables[x].indexOf('var') === 0 
                              ? 'v-badge-subtle' 
                              : ''
                          ].join(' ')}
                        >
          ...
          ? <div className="v-card v-card-body">
            <div className="v-message v-section">
              ...
              <div className="v-message-content">
              For more information, see our <Link route="#colors-and-palette">Colors and Palette guidelines</Link>
              </div>
            </div>
            <br />
                ...
                {Object.keys(docs.entries.themes[item].variables).map((x, i) => (
                  x.indexOf('--palette') === 0 
                    ? <tr className="v-tr" key={i}>
                      ...
                      <td className="v-td">
                        <div className="w-color-head">
                          <div className="w-color-box">
                            <div className="w-color-box-value" style={{ 
                      ...
                      <td className="v-td">{capitalCase(x.replace('--palette', ''))}</td>
                    </tr>
                    : null 
                ))}
              </tbody>
          ...
          : selectedTab === 'graph'
          ? <>
            <div className="v-flex v-flex-col v-gap-4" style={{'width': '300px'}}>
              <label className="v-label" htmlFor="token-relationships">Select token category</label>
              <div className="v-input-container v-surface v-flex">
                <select 
                  ...
                  onChange={(e) => { setRelationalGraphSegment(e.target.value) }}
                >
            ...
            {relationalGraphData.length > 0
              ? <AlluvialDiagram 
                ...
                cursor="pointer"
                dataLabel={{"visible":true,"placement":"inside","labelAccessor":"id", "format": "", "collisionHideOnly":false,"collisionPlacement":"top"}}
                groupAccessor="group"
                onHoverEvent={event => setHoverLink({...event.detail.data, group: "highlight"})}
                ...
                nodeConfig={{"alignment":"left","compare":true,"fill":false,"padding":relationalGraphSegment === 'all' ? 20 : 2,"width":relationalGraphSegment === 'all' ? 12 : 8}}
